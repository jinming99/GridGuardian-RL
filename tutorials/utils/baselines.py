"""
Baseline algorithms for EVChargingEnv.

Original source: SustainGym (https://github.com/sustainability-gym/sustaingym)
This adapted version uses GridGuardian-RL local environments and utilities.
"""
...
# Use local BaseAlgorithm and local EV env APIs
from algorithms.base import BaseAlgorithm
from envs.evcharging import EVChargingEnv  # type: ignore
from envs.evcharging.env import magnitude_constraint  # type: ignore

# Provide a solver shim compatible with prior monkey-patching in tutorials
# Expose the name `solve_mosek` but implement with a robust fallback
from tutorials.utils.common import solve_cvx_with_fallback as solve_mosek
...
class GreedyAlgorithm(BaseAlgorithm):
    ...
    def __init__(self, env: EVChargingEnv):
        if env is None:
            raise ValueError(
                "GreedyAlgorithm received env=None. Create an environment first, e.g., "
                "env = create_ev_env(...), and pass that instance."
            )
    ...
    def get_action(self, observation: dict[str, Any]) -> np.ndarray:
        ...
        return np.where(observation['demands'] > 0, self.max_action, 0).astype(np.float32)
...
class MPC(BaseAlgorithm):
    ...
    def __init__(self, env: EVChargingEnv, lookahead: int = 12):
        if env is None:
            raise ValueError(
                "MPC received env=None. Create an environment first, e.g., "
                "env = create_ev_env(...), and pass that instance."
            )
        super().__init__(env)
    ...
    def get_action(self, observation: dict[str, Any]) -> np.ndarray:
        ...
        for i in range(self.env.num_stations):
            ...
            mask[i, :int(cur_est_dep[i])] = MAX_ACTION
        self.mask.value = mask

        solve_mosek(self.prob)
        return np.asarray(self.traj.value[:, 0], dtype=np.float32)  # take first action
...
class OfflineOptimal(BaseAlgorithm):
    ...
    def __init__(self, env: EVChargingEnv):
        if env is None:
            raise ValueError(
                "OfflineOptimal received env=None. Create an environment first, e.g., "
                "env = create_ev_env(...), and pass that instance."
            )
        super().__init__(env)
        assert isinstance(env.action_space, spaces.Box), \
