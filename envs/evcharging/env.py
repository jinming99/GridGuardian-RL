...
# Optional notebook logger: fall back to print if unavailable
try:  # pragma: no cover - optional dependency
    from tutorials.utils.notebook import log_info as _nb_log_info
except Exception:  # pragma: no cover
    def _nb_log_info(msg: str) -> None:  # type: ignore
        print(msg)
...
class EVChargingEnv(Env):
    ...
    def __init__(self, data_generator: AbstractTraceGenerator,
        ...
        # # print(f"my shape: {[v.low for v in self.observation_space.values()]}")

##!!!! the box part is for SAFE_RL       !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
        if self.safe_rl:  
...
###!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
    ...
    def step(self, action: np.ndarray
        ...
        # Normalize action dtype/shape early to avoid dtype casting issues when adding noise
        # and to ensure downstream scaling/clipping operates on float arrays.
        action = np.asarray(action, dtype=np.float32).reshape(-1)

        #!!!!!!!!!!!!!!!!!!!!!!!!!!!!! action noise part !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
        if self.noise_action is not None and self.noise_action > 0:
            # print(f"Action after projection: {action}") 
            if self.verbose >= 2:
                _nb_log_info(f"noise_action level: {self.noise_action}")
        
            # Add noise to the projected action for robustness testing
            # Generating Gaussian noise (mean=0, std_dev=0.1) with the same shape as action
            noise = np.random.normal(0, self.noise_action, size=action.shape) * 5 #! to get the value here, it was multiplied by the action
            action += noise  # Add noise to the action

            #Print the action after adding noise for debugging purposes
            #print(f"Action after projection and noise: {action}")
        ...
        ###!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!! start of changes for obs noise !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!

        if self.noise is not None and self.noise > 0:
            noise_level_int = int(self.noise * 100)
            if self.verbose >= 2:
                _nb_log_info(f"noise level: {self.noise}")
                _nb_log_info(f"noise_level_int: {noise_level_int}")
            ## prev_moer
            # print(f"before noise prev_moer: {observation['prev_moer']}")
            prev_moer = observation["prev_moer"].copy()
            for i in range(len(prev_moer)):
                prev_moer[i] = observation["prev_moer"][i]
            noise = np.random.normal(0, self.noise, size=prev_moer.shape) 
            prev_moer = prev_moer + noise
            prev_moer = np.clip(prev_moer, 0, 1)
            for i in range(len(prev_moer)):
                observation["prev_moer"][i] = prev_moer[i]
            # print(f"after noise prev_moer: {observation['prev_moer']}")
        
            ## est_departures
            # print(f"before noise est_departures: {observation['est_departures']}")
            est_departures = observation["est_departures"].copy()
            for i in range(len(est_departures)):
                est_departures[i] = observation["est_departures"][i]
            if noise_level_int > 0:
                noise = np.random.randint(
                    -noise_level_int, noise_level_int, size=est_departures.shape
                )
            else:
                noise = np.zeros_like(est_departures, dtype=int)
            est_departures = est_departures + noise
            est_departures = np.clip(est_departures, -288, 288)
            for i in range(len(est_departures)):
                if observation["est_departures"][i] != 0:
                    observation["est_departures"][i] = est_departures[i]
            # print(f"after noise est_departures: {observation['est_departures']}")

            ## demands
            # print(f"before noise demands: {observation['demands']}")
            demands = observation["demands"].copy()
            for i in range(len(demands)):
                demands[i] = observation["demands"][i]
            # print(f"demands: {demands}")
            noise = np.random.normal(0, self.noise, size=demands.shape) 
            demands = demands + noise
            # print(f"demand uzunluk: {len(demands)}")
            demands = np.clip(demands, 0, self.data_generator.requested_energy_cap)
            for i in range(len(demands)):
                if observation["demands"][i] != 0:
                    observation["demands"][i] = demands[i]
            # print(f"after noise demands: {observation['demands']}")

            ## forecasted_moer
            # print(f"before noise forecasted_moer: {observation['forecasted_moer']}")
            forecasted_moer = observation["forecasted_moer"].copy()
            for i in range(len(forecasted_moer)):
                forecasted_moer[i] = observation["forecasted_moer"][i]
            noise = np.random.normal(0, self.noise, size=forecasted_moer.shape)  
            forecasted_moer = forecasted_moer + noise
            forecasted_moer = np.clip(forecasted_moer, 0, 1)
            for i in range(len(forecasted_moer)):
                observation["forecasted_moer"][i] = forecasted_moer[i]
            # print(f"after noise forecasted_moer: {observation['forecasted_moer']}")



        ###!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!! end of changes for obs noise !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
    ...
    def reset(self, *, seed: int | None = None, options: dict[str, Any] | None = None
        ...
        if self.verbose >= 1:
            _nb_log_info(f'Simulating {num_plugs} events using {self.data_generator}')
    ...
    def _to_schedule(self, action: np.ndarray) -> dict[str, list[float]]:
        ...
        # Ensure action is a 1D float array to avoid ambiguous truth values
...
# !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!! Cost Function !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
        ...
        self.carbon_cost = self.CARBON_COST_FACTOR * total_charging_rate * self.moer[self.t, 0]

        # total_reward = profit - self.carbon_cost - self.excess_charge 
        # total_reward = profit - self.excess_charge 
        total_reward = profit - self.carbon_cost 

        # Update reward information-tracking
        self._reward_breakdown['profit'] += profit
        self._reward_breakdown['carbon_cost'] += self.carbon_cost 
        self._reward_breakdown['excess_charge'] += self.excess_charge 
# !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
