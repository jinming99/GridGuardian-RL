...
# Optional notebook logger: fall back to console prints when unavailable
try:  # pragma: no cover - optional dependency
    from tutorials.utils.notebook import log_info as _nb_log_info, log_warning as _nb_log_warning
except Exception:  # pragma: no cover
    def _nb_log_info(msg: str) -> None:  # type: ignore
        print(msg)
    def _nb_log_warning(msg: str) -> None:  # type: ignore
        print(msg)
...
def create_gmm(site: SiteStr, n_components: int,
    ...
    df = df[df['claimed']]
    if len(df) == 0:
        _nb_log_warning('Empty dataframe, abort GMM training.')
        return
    ...
    # Train and save
    _nb_log_info(
        f'Fitting GMM ({n_components} components, {len(df.columns)} dimensions) '
        f'on data from {site} site from {date_range[0].strftime(DATE_FORMAT)} '
        f'to {date_range[1].strftime(DATE_FORMAT)}...'
    )
